...
if [ -n "$WORDPRESS_CONTAINER_NAME" ]; then
    ...
    LATEST_FILE=$(oc exec -n $NAMESPACE -c $WORDPRESS_CONTAINER_NAME $WORDPRESS_POD_NAME -- bash -c 'ls -t /var/www/html/wp-content/ai1wm-backups-history | head -n '"$BACKUP_NUMBER")
