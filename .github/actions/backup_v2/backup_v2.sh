...
set +e
CMD1_RESULTS=$( (oc exec -n $NAMESPACE -c $DB_CONTAINER_NAME $DB_POD_NAME -- sh -c 'mariadb-dump  -u root -p$(cat $MYSQL_ROOT_PASSWORD_FILE) --quick --extended-insert  $MYSQL_DATABASE | gzip' > db.sql.gz) 2>&1) #--max-allowed-packet=500M --net-buffer-length=16M
CMD1_EXIT_CODE=$?
set -e
