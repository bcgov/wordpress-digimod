...
if [[ "$CMD1_EXIT_CODE" -eq 0 && -f "$S3_FILENAME" ]]; then 
    ...
    if [ "$RESTORE_DB" = "true" ]; then
        ...
        CMD1_RESULTS=$( pv db.sql | (oc exec -i -n $NAMESPACE -c $DB_CONTAINER_NAME $DB_POD_NAME -- sh -c "mariadb  -u root -p$(cat $MYSQL_ROOT_PASSWORD_FILE) $MYSQL_DATABASE --init-command='SET GLOBAL innodb_flush_log_at_trx_commit=2; SET GLOBAL innodb_doublewrite=0; SET GLOBAL foreign_key_checks=0; SET GLOBAL unique_checks=0; SET GLOBAL autocommit=0;'") 2>&1)
        CMD1_EXIT_CODE=$?
        set -e
